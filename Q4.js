...
function discountCalculate(price) {
    if (price > 100) {
        let discountPrice = (price * 10) / 100;
        let remainingPrice = price - discountPrice;
        console.log(`The original price of product is ${price}$, So we have applied 10 % discount that the amount is : ${discountPrice}`);
    }
    else {
        let discountPrice = (price * 5) / 100;
        let remainingPrice = price - discountPrice;
        console.log(`The original price of product is ${price} $, So we have applied 5 % discount that the amount is : ${discountPrice}`);
    }
}
